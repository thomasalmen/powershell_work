...
when RULE_INIT {
    ...
    set static::close_start_date "$startDate"

    #End maintenance window in YYYY-mm-dd HH:MM format
    set static::close_end_date "$endDate"
    ...
    set static::close_start [clock scan `$static::close_start_date]
    set static::close_end [clock scan `$static::close_end_date]
	...
	<div class="content">
	    ...
	    <p>
                ...
                <p>SCB st&auml;nger samtliga system f&ouml;r underh&aring;ll [clock format `$static::close_start -format {%Y-%m-%d, kl %H:%M} ].
                <br>
                Vi &ouml;ppnar igen [clock format `$static::close_end -format {%Y-%m-%d, kl %H:%M} ].
                </p>
        ...
        <p>
                ...
                <p>Statistics Sweden will carry out systems-wide maintenance starting [clock format `$static::close_start -format {%Y-%m-%d, at %H:%M} ]. 
                <br>
                The website will be available again [clock format `$static::close_end -format {%Y-%m-%d, at %H:%M} ].
                </p>
...
when HTTP_REQUEST priority 1 {
    ...
    if { [clock seconds] > `$static::close_start  and [clock seconds] < `$static::close_end } {
